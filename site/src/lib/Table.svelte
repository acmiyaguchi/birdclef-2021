<script>
  ...
  // NOTE: autocolumns may be a little overkill for this project
  $: columns = (columns || (data.length > 0 && autoColumns(data))).map((row) => ({
    format: (col) => (col[row.name] == undefined ? "" : col[row.name]),
    html: false,
    ...row
  }));
